...
class Boss(pygame.sprite.Sprite):
    def __init__(self, x, y, img, health, quantity, img_attacks, damage):
        ...
        self.attack = pygame.transform.scale(img_attacks, (img.get_width() * 1.5, img.get_height() * 1.5))
    ...
    def update(self, *args):
        ...
        if self.clock < pygame.time.get_ticks():
            ...
            if abs(hero.rect.x - x) > 100:
                ...
                self.speed = 0
                self.attack_image += 1
                if self.attack_image + 1 > 14:
                    self.attack_image = 0
                if self.last_side == 0:
                    self.attack = args[10][self.attack_image]

                elif self.last_side == 1:
                    self.attack = args[11][self.attack_image]

                elif self.last_side == 2:
                    self.attack = args[9][self.attack_image]

                elif self.last_side == 3:
                    self.attack = args[8][self.attack_image]

                if (self.attack_image == 13):
                    hero.take_damage(self.damage)

                self.attack = pygame.transform.scale(self.attack,(self.attack.get_width() * 2, self.attack.get_height() * 2))
                self.speed = 5 + (500 - self.health) / 35
                self.damage = 10 + (12 - self.damage) / 10
            if self.last_side == 0:
            ...
            pygame.draw.line(screen, (255, 0, 0), [self.rect.x - 80, self.rect.y - 10],[self.rect.x -80 + self.health / 1.7, self.rect.y - 10], 9)
